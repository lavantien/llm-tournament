...
Use this file as the primary instruction set for this workspace (Gemini CLI v0.15+).

## Coding Standards & TDD
- **Strict TDD:** Follow test-driven development rigorously.
- **Red-Green Loop:** Always write failing tests *before* implementing code.
- **Guardrails:** Respect `TDD-Guard` hooks; do not force edits if tests are failing.
- **Verification:** Run the linter after every significant change to ensure correctness.

## Model Routing & Roles
- **Planning (Architect):** Use **Gemini Pro with Extensive Thinking** for high-level reasoning, complex architecture, and "thinking" steps.
- **Execution (Builder):** Use **Gemini Pro with Non-Thinking** or **Gemini Flash** for the majority of code editing, refactoring, and command execution. It is currently the state-of-the-art for agentic coding.
- **Swarm Operations:** Use **Gemini Flash** or **Gemini Flash-Lite** for low-latency sub-agents, parallel execution, or voting swarms.

## Complex Task Workflow
- **Context & Docs:** Use the `/mcp` command to leverage **Context7 MCP (from MCP_DOCKER)** or **Google Search Grounding** or **Web Fetching** to fetch up-to-date documentation.
- **Swarm Protocol:** For deadlocks, initiate a Council Swarm using the MAKER/DAP protocol (detailed below) utilizing multiple Flash instances for voting and Deep Think for synthesis.
- **Error Recovery:**
    - If stuck after three failed attempts, use `/undo` (Gemini CLI native command) to revert the context state.
    - If deep in a git hole, use `git reset --hard`.

## Project Constraints
- **Go Race Detection:** For any `go run` or `go test` commands, **always** prefix with `CGO_ENABLED=1` (e.g., `CGO_ENABLED=1 go test -race ./...`).
- **Generated Code:** Never edit files in `gen/` directories. Run `go generate` to rebuild from OpenAPI/Protobuf specs.
- **Clean Output:** Do not include "Generated by Gemini" watermarks or "Co-Authored-By" lines.

## MAKER/DAP Protocol (Decomposition & Atomic Processing)
**Trigger:** Invoke this protocol when facing complex features, persistent bugs, or getting stuck.

### Core Principle
Do not attempt to solve the whole problem at once. Replace "smart" reasoning with "extreme" decomposition and verification.

### Phase 1: MAKER (Extreme Decomposition)
1. **Stop Coding:** Halt all execution.
2. **Atomize (Deep Think):** Use **Gemini Pro Deep Think** to break the problem into the smallest possible atomic subtasks.
3. **Clarity:** Each subtask must be unambiguous (e.g., "Create file X," "Define struct Y," "Write function signature Z").
4. **List:** Explicitly list these subtasks in the chat context.

### Phase 2: DAP/TDD (Micro-Agent Execution)
1. **Pick & Isolate:** Select the first subtask.
2. **Test First (Red):** Write *only* the specific test case for this tiny subtask. Run it -> Confirm Failure.
3. **Minimal Implementation (Green):** Write *only* the code necessary to pass that specific test using **Gemini Pro**.
4. **Verify & Vote:** Check if the output matches the subtask goal exactly. If not, discard and retry immediately.
5. **Refactor:** Run linters and clean up, ensuring tests still pass.

### Phase 3: Iterate
1. Mark the subtask as **DONE**.
2. Move to the next subtask.
3. **Recursive Break:** If a subtask becomes difficult, recurse: break it down further until it is trivial.

### Rule of Thumb
If you are guessing or writing more than 10 lines of logic without a test, you have broken the protocol. Decompose, Test, Implement, Verify.
...
- In case of strange errors or uncertainty, use `/mcp` or Google Search Grounding to fetch latest information instead of guessing
...
- If encountering strange errors or uncertainty, use `/mcp` or Google Search for latest information
...
- **When Adding Tests:** Follow TDD approach from instructions above
...
1. **Design Phase:**
   ...
   - Use **Gemini Pro with Extensive Thinking** for architectural decisions
...
2. **Implementation Phase (MAKER/DAP Protocol for Complex Features):**
   - Break into smallest atomic subtasks using **Gemini Pro Deep Think**
   - Write test for each subtask (if TDD approach)
   - Implement minimal code to pass test using **Gemini Pro**
   - Verify and refactor
...
4. **Documentation Phase:**
   ...
   - Update this GEMINI.md if adding new patterns or conventions
