...
func TestImportErrorHandling(t *testing.T) {
	...
	// Test import prompts error handling
	resp, err := http.PostForm(ts.URL+"/import_prompts", url.Values{
		"prompts_file": {""}, // Empty file to trigger error
	})
	if err != nil {
		t.Fatalf("Failed to send POST request: %v", err)
	}
	...
	// Check if the response redirects to the import error page
	...
	// Test import results error handling
	resp, err = http.PostForm(ts.URL+"/import_results", url.Values{
		"results_file": {""}, // Empty file to trigger error
	})
	if err != nil {
		t.Fatalf("Failed to send POST request: %v", err)
	}
