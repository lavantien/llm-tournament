...
class PositionEmbeddingSine:
    ...
    def __call__(self, b, h, w):
        ...
        dim_t = self.temperature ** (2 *
                                     (self.dim_t.to(device) // 2) / self.num_pos_feats)
...
class MCLM(nn.Module):
    ...
    def forward(self, l, g):
        ...
        concated_locs = rearrange(
            l, '(hg wg b) c h w -> b c (hg h) (wg w)', hg=2, wg=2)
        ...
        g_hw_b_c = g_hw_b_c + \
            self.dropout1(self.attention[0](
                g_hw_b_c + self.g_pos, pools + self.p_poses, pools)[0])
        g_hw_b_c = self.norm1(g_hw_b_c)
        g_hw_b_c = g_hw_b_c + \
            self.dropout2(self.linear2(self.dropout(
                self.activation(self.linear1(g_hw_b_c)).clone())))
        ...
        for i, (_l, _g) in enumerate(zip(l_hw_b_c.chunk(4, dim=1), _g_hw_b_c.chunk(4, dim=1))):
            outputs_re.append(
                self.attention[i + 1](_l, _g, _g)[0])  # (h w) 1 c
        outputs_re = torch.cat(outputs_re, 1)  # (h w) 4 c

        l_hw_b_c = l_hw_b_c + self.dropout1(outputs_re)
        l_hw_b_c = self.norm1(l_hw_b_c)
        l_hw_b_c = l_hw_b_c + \
            self.dropout2(self.linear4(self.dropout(
                self.activation(self.linear3(l_hw_b_c)).clone())))
...
class MCRM(nn.Module):
    ...
    def forward(self, x):
        ...
        patched_glb = rearrange(
            glb, 'b c (hg h) (wg w) -> (hg wg b) c h w', hg=2, wg=2)
        ...
        loc = loc * rearrange(token_attention_map,
                              'b c (hg h) (wg w) -> (hg wg b) c h w', hg=2, wg=2)
...
class BEN_Base(nn.Module):
    def __init__(self):
        ...
        self.backbone = SwinTransformer(embed_dim=128, depths=[2, 2, 18, 2], num_heads=[
                                        4, 8, 16, 32], window_size=12)
        emb_dim = 128
    ...
    def forward(self, x):
        ...
        for i in range(real_batch):
            ...
            final_output = rescale_to(
                final_output + resize_as(shallow, final_output))
    ...
    def inference(self, image, refine_foreground=False):
        ...
        if isinstance(image, Image.Image):
            ...
            if torch.cuda.is_available():
                ...
                img_tensor = img_transform(image).unsqueeze(
                    0).to(next(self.parameters()).device)
            else:
                img_tensor = img_transform32(image).unsqueeze(
                    0).to(next(self.parameters()).device)
            ...
            if refine_foreground == True:
                ...
                image_masked = refine_foreground_process(
                    original_image, pred_pil)
    ...
    def segment_video(self, video_path, output_path="./", fps=0, refine_foreground=False, batch=1, print_frames_processed=True, webm=False, rgb_value=(0, 255, 0)):
        ...
        while True:
            ...
            if not ret:
                if batch_frames:
                    batch_results = self.inference(
                        batch_frames, refine_foreground)
                    ...
                    if print_frames_processed:
                        print(f"Processed frames {
                              frame_idx-len(batch_frames)+1} to {frame_idx} of {total_frames}")
                break
            ...
            if len(batch_frames) == batch:
                ...
                if print_frames_processed:
                    print(f"Processed frames {
                          frame_idx-batch+1} to {frame_idx} of {total_frames}")
                batch_frames = []
        ...
        if webm:
            ...
            pil_images_to_webm_alpha(
                foregrounds, alpha_webm_path, fps=original_fps)

        else:
            cap.release()
            fg_output = os.path.join(output_path, 'foreground.mp4')

            pil_images_to_mp4(foregrounds, fg_output,
                              fps=original_fps, rgb_value=rgb_value)
...
def refine_foreground_process(image, mask, r=90):
    ...
    image_masked = Image.fromarray(
        (estimated_foreground * 255.0).astype(np.uint8))
    return image_masked
